...
class Joystick(gobject.GObject):
    ...
    def read_buttons(self):
        ...
        time, value, event_type, number = struct.unpack(self.EVENT_FORMAT, read_event)
        # get just the button/axis press event from the event event_type
        event = event_type & ~self.EVENT_INIT
        # get just the INIT event from the event event_type
        init = event_type & ~event
        signal = None
